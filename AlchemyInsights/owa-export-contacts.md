---
title: '8000020: Slik eksporterer du kontakter'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: nb-NO
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983282"
---
# <a name="export-contacts"></a>Eksportere kontakter

1. Nederst på siden i Outlook på nettet velger du <img src='data:image/png;base64,' />
 **Personer**.
2. Velg Behandle eksporter kontakter på   >  **verktøylinjen.** 
3. Velg å eksportere alle kontakter eller bare kontakter fra en bestemt mappe, og velg deretter **Eksporter**.
4. Velg Lagre nederst på  siden for å lagre «contacts.csv» i standard nedlastingsmappe. Du kan også velge pilen ved siden **av Lagre og** velge Lagre **som**.

Hvis du vil ha mer informasjon, kan du se Eksportere [kontakter fra Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

