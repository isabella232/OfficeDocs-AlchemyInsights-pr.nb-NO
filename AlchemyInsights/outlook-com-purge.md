---
title: Outlook.com-tømning
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "1964"
- "9000336"
ms.openlocfilehash: 82ee2a56741655be3070b09eeb4e1e423b648250154af7448e230fe2ef1c1950
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: nb-NO
ms.lasthandoff: 08/05/2021
ms.locfileid: "54095767"
---
# <a name="permanently-delete-email-in-outlookcom"></a>Slette e-post permanent på Outlook.com

Hvis du vil slette e-post eller tømme innboksen:

1. Merk meldingene du vil slette (eller velg sirkelen <img src='data:image/png;base64,' />
 ved siden **av Innboksen** øverst i meldingslisten for å velge alle meldingene i innboksen).
1. Øverst på siden velger du Slett **(for** hele mappen  **Tom** mappe , eller Tøm tøm andre med fokus hvis innboks med  /   fokus er aktivert).
1. Hvis du vil fjerne alle slettede e-postmeldinger, åpner du Slettede elementer-mappen og velger Tøm mappe **,** velger deretter Gjenopprettelige **elementer** øverst på siden og **velger Tøm mappe på** nytt. [](https://outlook.live.com/mail/deleteditems)

Hvis du vil ha mer informasjon, kan du lese [Slette e-post på Outlook.com](https://support.office.com/article/a9b63739-5392-412a-8e9a-d4b02708dee4?wt.mc_id=Office_Outlook_com_Alchemy).