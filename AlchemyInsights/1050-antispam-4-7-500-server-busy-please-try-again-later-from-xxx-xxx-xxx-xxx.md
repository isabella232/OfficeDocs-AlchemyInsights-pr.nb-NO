...
ms.locfileid: "29915381"
---
# <a name="47500-server-busy-please-try-again-later"></a><span data-ttu-id="bd01d-103">4.7.500 serveren er opptatt, prøv på nytt senere</span><span class="sxs-lookup"><span data-stu-id="bd01d-103">4.7.500 Server busy, please try again later</span></span>

<span data-ttu-id="bd01d-104">Denne feilen oppstår når volumet av e-post trafikk fra kilde-IP-adressen overskrider grensen basert på omdømme (eller mangel på rykte) av kildens IP-adresse.</span><span class="sxs-lookup"><span data-stu-id="bd01d-104">This error occurs when the volume of email traffic from the source IP address exceeds the limit based on the reputation (or lack of reputation) of source IP address.</span></span>
  
<span data-ttu-id="bd01d-p102">Blokkering av e-post fra IP-kildeadresse utløper innen en time. Hvis kildens IP-adresse er en lokale e-postserver som hører til deg, kan du kontrollere konfigurasjonen av e-post flyt koblingen. Hvis atferden vedvarer for mer enn en time, kan du kontakte kundestøtte for å be om et unntak for kildens IP-adresse.</span><span class="sxs-lookup"><span data-stu-id="bd01d-p102">Blocking email from the source IP address will expire within an hour. If the source IP address is an on-premises email server that belongs to you, verify the configuration of the mail flow connector. If the behavior continues for more than an hour, contact support to request an exception for the source IP address.</span></span>
